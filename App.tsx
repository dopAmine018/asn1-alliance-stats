...
const App: React.FC = () => {
  ...
  const handleTabChange = (tab: 'public' | 'admin') => {
    ...
    // Refresh settings when going between tabs to ensure latest visibility
    MockApi.getSettings().then(data => setSettings(prev => ({ ...prev, ...data })));
  };
    ...
    >
      {activeTab === 'public' && (
        <>
            ...
            {view === 'storm_public' && <DesertStormViewer onBack={() => setView('home')} onCreateProfile={() => setView('update')} />}
        </>
      )}
